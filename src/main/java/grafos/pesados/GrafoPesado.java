...
public class GrafoPesado {
    ...
    public boolean hayCiclo(GrafoPesado grafoAux, int posVertice) throws ExcepcionAristaYaExiste {
        ...
        for (AdyacentesConPeso adyacentes : adyacentesDeVertice) {
            if(!controlMarcados.estaVerticeMarcado(adyacentes.indiceVertice)) {
                if(!grafoAux.existeAdyacencia(posVertice, adyacentes.indiceVertice) && posVertice != adyacentes.indiceVertice) {
                    grafoAux.insertarArista(posVertice, adyacentes.indiceVertice,1);
                    hayCiclo(grafoAux, adyacentes.indiceVertice);
                }
            } else {
                if(!grafoAux.existeAdyacencia(posVertice, adyacentes.indiceVertice) && posVertice != adyacentes.indiceVertice) {
                    grafoAux.insertarArista(posVertice, adyacentes.indiceVertice,1);
